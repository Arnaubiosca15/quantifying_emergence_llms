...
# Configure logging
LOG_FILE = os.path.join(PROJECT_ROOT, "results", "untrained_pipeline.log")
...
def plot_emergence(results_list):
    ...
    plt.title('Untrained GPT-2: Baseline Causal Emergence vs Imbalance', fontsize=14, fontweight='bold')
    ...
    plt.ylim(-0.1, 0.5)
    plot_path = os.path.join(FIGURES_DIR, "emergence_vs_imbalance_untrained.png")
    plt.tight_layout()
...
def main():
    logger.info("Initializing Untrained Pipeline (Random Weights)...")

    logger.info("Loading architecture and destroying weights for UNTRAINED GPT-2...")
    model = EasyTransformer.from_pretrained("gpt2").cuda()
    
    # Randomly reinitialize all weights to destroy any pre-trained knowledge
    with torch.no_grad():
        for name, param in model.named_parameters():
            if 'weight' in name and param.dim() >= 2:
                ...
                torch.nn.init.normal_(param, mean=0.0, std=0.02) # Embeddings/LayerNorm weights
    ...
    for ratio in IMBALANCE_RATIOS:
        ...
        csv_path  = os.path.join(CSV_DIR, f"tpm_logits_untrained_{ratio}_{100-ratio}.csv")
