...
# Configurar logging
LOG_FILE = os.path.join(PROJECT_ROOT, "results", "nim_inference_large.log")
...
def main():
    logger.info("Initializing Nim Inference & CE Pipeline (LARGE MODEL)...")
    
    if not os.path.exists(INPUT_JSON):
        logger.error(f"Dataset not found at {INPUT_JSON}. Run generator first.")
        return

    logger.info("Loading GPT-2 LARGE model on CUDA. This might take a moment...")
    ...
    # ==========================================
    logger.info("\n=== RESULTS: CAUSAL EMERGENCE IN NIM (GPT-2 LARGE) ===")
    ...
    axes[0].set_title("Nivel Micro (GPT-2 Large)", fontsize=14, fontweight="bold")
    ...
    axes[1].set_title("Nivel Macro (GPT-2 Large)", fontsize=14, fontweight="bold")
    axes[1].set_ylabel("Macro Estado Actual")
    axes[1].set_xlabel("Macro Estado Futuro")
    
    plt.tight_layout()
    plot_path_tpm = os.path.join(FIGURES_DIR, "nim_tpm_heatmaps_large.png")
    ...
    plt.title('Emergencia Causal (Nim) - GPT-2 Large', fontsize=15, fontweight='bold')
    ...
    plot_path_bar = os.path.join(FIGURES_DIR, "nim_causal_emergence_large.png")
